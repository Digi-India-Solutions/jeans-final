...
function CreateOrderModal({ subProducts, orders, setOrders, setFilteredOrders, filteredOrders, setOrderToPrint, openProductSelection, setShowPrintOrderModal, setShowProductSelectionModal, calculatePointsValue, calculatePointsToEarn, getTotalPaidAmount, setShowCreateOrderModal, setNewOrderForm, newOrderForm }) {
    ...
    const getTotalPcs = () => {
        return newOrderForm?.items?.reduce((sum, item) => {
            return sum + (item?.quantity * (item?.pcsInSet || 0));
        }, 0);
    };
                                        ...
                                        <div className="space-y-4 p-4 bg-orange-50 rounded-lg">
                                            ...
                                            <div className="flex items-center space-x-3">
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <input
                                                            type="number"
                                                            value={newOrderForm?.redeemPoints}
                                            ...
                                            {newOrderForm?.redeemPoints > 0 && (
                                                <div className="text-xs text-orange-700 bg-orange-200 p-3 rounded">
                                                    <div className="flex justify-between mb-1">
                                                        <span>Redeeming:</span>
                                                        <span className="font-medium">{newOrderForm?.redeemPoints?.toLocaleString()} points</span>
                                                    </div>
                                                    <div className="flex justify-between mb-1">
                                                        <span>Discount:</span>
                                                        <span className="font-medium">₹{calculatePointsValue(newOrderForm?.redeemPoints)?.toFixed(2)}</span>
                                                    </div>
                                                    <div className="flex justify-between font-bold">
                                                        <span>Payable After Discount:</span>
                                                        <span>₹{(getTotalValue() - calculatePointsValue(newOrderForm?.redeemPoints))?.toLocaleString()}</span>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    {newOrderForm?.redeemPoints > 0 && (
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-medium text-green-600">-₹{calculatePointsValue(newOrderForm?.redeemPoints).toFixed(2)}</span>
                                        </div>
                                    )}
                                    ...
                                    <div className="border-t pt-3">
                                        <div className="flex justify-between font-medium">
                                            ...
                                            <span className="text-lg text-blue-600">₹{(getTotalValue() - calculatePointsValue(newOrderForm?.redeemPoints))?.toLocaleString()}</span>
                                        </div>
                                    </div>
                                    ...
                                    {getTotalValue() > 0 && (
                                        <div className="mt-4 pt-3 border-t">
                                            <div className="flex justify-between font-medium">
                                                ...
                                                <span className="text-orange-600">{calculatePointsToEarn(getTotalValue() - calculatePointsValue(newOrderForm?.redeemPoints))} pts</span>
                                            </div>
